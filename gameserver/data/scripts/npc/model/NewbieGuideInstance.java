			...
			if(player.getClassId().getLevel() == 1)
			{
				if(player.getVar("NewGuidReward") == null)
				{
					...
					if(isMage)
					{
						...
						ItemFunctions.addItem(player, 5790, 100); // Spiritshot
					}
					else
					{
						player.sendPacket(new PlaySound("tutorial_voice_026"));
						ItemFunctions.addItem(player, 5789, 200); // Soulshot
					}
					ItemFunctions.addItem(player, 8594, 2); // Recovery Scroll: NG
				...
				if(player.getLevel() < 6) // FIXME: если получить 6 левел во время квеста то награду не дадут
					if(player.isQuestCompleted(1) || player.isQuestCompleted(2) || player.isQuestCompleted(4) || player.isQuestCompleted(5) || player.isQuestCompleted(166) || player.isQuestCompleted(174))
					{
						...
						player.sendPacket(new HtmlMessage(this, "newbiehelper/q1-2.htm"));
						return;
					}
					else
					{
						player.sendPacket(new HtmlMessage(this, "newbiehelper/q1-1.htm").replace("%tonpc%", getQuestNpc(1, player)));
						return;
					}
				if(player.getLevel() < 10)
					if(player.getVarB("p1q2"))
					{
						...
						player.sendPacket(new HtmlMessage(this, "newbiehelper/q3-1.htm").replace("%tonpc%", getQuestNpc(3, player)));
						return;
					}
					else
					{
						player.sendPacket(new HtmlMessage(this, "newbiehelper/q2-1.htm").replace("%tonpc%", getQuestNpc(2, player)));
						return;
					}
				if(player.getLevel() < 15)
					if(player.getVarB("p1q3"))
					{
						...
						player.sendPacket(new HtmlMessage(this, "newbiehelper/q4-1.htm").replace("%tonpc%", getQuestNpc(4, player)));
						return;
					}
					else
					{
				...
				if(player.getLevel() < 18)
					if(player.getVarB("p1q4"))
					{
						...
						player.sendPacket(new HtmlMessage(this, "newbiehelper/q4-2.htm"));
						return;
					}
					else
					{
				...
				player.sendPacket(new HtmlMessage(this, "newbiehelper/q-no.htm"));
				return;
			}
			else
			{
