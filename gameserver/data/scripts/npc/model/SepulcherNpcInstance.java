...
public class SepulcherNpcInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		{
			long count = ItemFunctions.getItemCount(player, HALLS_KEY);
			if(count == 0)
			...
			else if(FourSepulchersManager.isAttackTime())
			{
				...
				if(player.getParty() != null)
					...
					{
						ItemFunctions.deleteItem(mem, HALLS_KEY, ItemFunctions.getItemCount(mem, HALLS_KEY));
					}
				else
					ItemFunctions.deleteItem(player, HALLS_KEY, count);
			}
		}
		else
