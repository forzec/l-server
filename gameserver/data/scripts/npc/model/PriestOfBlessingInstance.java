...
public class PriestOfBlessingInstance extends NpcInstance
{
	...
	{
		if(!Config.ALT_PRIEST_BLESSING_ENABLED)
			val = 3;
		else if(val == 0)
		{
			Hourglass hg = getHourglass(player);
	...
	private void buyLimitedItem(Player player, String var, int itemId, int price, boolean isGlobalVar)
	{
		...
		if(isGlobalVar)
		{
			IntObjectMap<AccountPlayerInfo> chars = player.getAccountChars();
			long use_time = 0;

			for(int objId : chars.keySet().toArray())
			{
		...
		if(_remaining_time >= _reuse_time)
		{
			if(player.reduceAdena(price, true))
			{
				...
				player.sendPacket(new SystemMessage(SystemMsg.S2_UNITS_OF_THE_ITEM_S1_ISARE_REQUIRED).addItemName(ItemTemplate.ITEM_ID_ADENA).addNumber(price));
		}
		else
		{
			...
			if(hours > 0)
				player.sendPacket(new SystemMessage(SystemMsg.THERE_ARE_S1_HOURSS_AND_S2_MINUTES_REMAINING_UNTIL_THE_ITEM_CAN_BE_PURCHASED_AGAIN).addNumber(hours).addNumber(minutes));
			else if(minutes > 0)
				player.sendPacket(new SystemMessage(SystemMsg.THERE_ARE_S1_MINUTES_REMAINING_UNTIL_THE_ITEM_CAN_BE_PURCHASED_AGAIN).addNumber(minutes));
