...
import org.mmocore.gameserver.network.l2.c2s.RequestSetCrop;
import org.mmocore.gameserver.network.l2.c2s.RequestSetSeed;
...
import org.mmocore.gameserver.network.l2.s2c.ExShowCropSetting;
...
import org.mmocore.gameserver.network.l2.s2c.ExShowSeedSetting;
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if(actualCommand.equalsIgnoreCase("PutTreasure"))
		{
			if(!val.equals(""))
			{
				...
				if(treasure > 0)
				{
					if(!player.reduceAdena(treasure, true))
					{
		...
		else if(actualCommand.equalsIgnoreCase("fortressStatus"))
		{
			...
			for(IntObjectPair<List> entry : castle.getRelatedFortresses().entrySet())
			{
				NpcString type;
				switch(entry.getKey())
				{
		...
		else if(actualCommand.equalsIgnoreCase("Crown")) // Give Crown to Castle Owner
		{
			...
			if(ItemFunctions.getItemCount(player, 6841) == 0)
			{
				ItemFunctions.addItem(player, 6841, 1);
