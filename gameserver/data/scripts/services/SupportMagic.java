...
public class SupportMagic
{
	...
	@Bypass("services.SupportMagic:getSupportMagic")
	public void getSupportMagic(Player player, NpcInstance npc, String[] arg)
	{
		doSupportMagic(npc, player, false);
	}

	@Bypass("services.SupportMagic:getSupportServitorMagic")
	public void getSupportServitorMagic(Player player, NpcInstance npc, String[] arg)
	{
		doSupportMagic(npc, player, true);
	}

	@Bypass("services.SupportMagic:getProtectionBlessing")
	public void getProtectionBlessing(Player player, NpcInstance npc, String[] arg)
	{
		...
		npc.doCast(SkillTable.getInstance().getSkillEntry(5182, 1), player, true);
	}
	...
	public static void doSupportMagic(NpcInstance npc, Player player, boolean servitor)
	{
		...
		{
			Functions.show("default/newbie_nosupport_servitor.htm", player, npc);
			return;
		}
		else
		{
			if(lvl < minSupLvl)
			{
				Functions.show("default/newbie_nosupport_min.htm", player, npc);
				return;
			}
			if(lvl > maxSupLvl || player.getClassId().getLevel() > 3)
			{
				Functions.show("default/newbie_nosupport_max.htm", player, npc);
				return;
			}
		}
		...
		if(servitor)
		{
			...
			for(int[] buff : _summonBuff)
				if(lvl >= buff[0] && lvl <= buff[1])
				{
					...
					npc.callSkill(SkillTable.getInstance().getSkillEntry(buff[2], buff[3]), target, true);
				}
		}
		else
		{
