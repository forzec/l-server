...
public class SeedOfAnnihilation implements OnInitScriptListener
{
	...
	private void startEffectZonesControl()
	{
		for(SeedRegion sr : _regionsData)
		{
			...
			changeZoneActive(sr.buff_zone_pc[sr.activeBuff - 1], true);
			changeZoneActive(sr.buff_zone_npc[sr.activeBuff - 1], true);
		}
	...
	private class ChangeSeedsStatus extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			{
				sr = _regionsData[i];
				int oldBuff = sr.activeBuff;
				sr.activeBuff = ZONE_BUFFS_LIST[nextBuffs][i];

				for(NpcInstance af : sr.af_npcs)
					af.setNpcState(sr.activeBuff);

				changeZoneActive(sr.buff_zone_pc[oldBuff - 1], false);
				changeZoneActive(sr.buff_zone_npc[oldBuff - 1], false);
				changeZoneActive(sr.buff_zone_pc[sr.activeBuff - 1], true);
				changeZoneActive(sr.buff_zone_npc[sr.activeBuff - 1], true);
			}
			ThreadPoolManager.getInstance().schedule(new ChangeSeedsStatus(), _seedsNextStatusChange - System.currentTimeMillis());
	...
	private void changeZoneActive(String zoneName, boolean val)
	{
	...
	{
		int old = ServerVariables.getInt("SeedBuffsList", 0);
		int result = Rnd.get(ZONE_BUFFS_LIST.length - 1);
		if (result >= old)
			result++;
		ServerVariables.set("SeedBuffsList", result);
		Log.add("Seed of Annihilation buff pattern changed from " + old + " to " + result, "seeds");
		return result;
	}
