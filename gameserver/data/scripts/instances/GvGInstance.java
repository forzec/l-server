...
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameAddPlayer;
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameChangePoints;
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameCloseUI;
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameEnd;
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameExtendedChangePoints;
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameRemovePlayer;
...
public class GvGInstance extends Reflection
{
	...
	private void end()
	{
		...
		//TODO [VISTALL] GvG.updateWinner(isRedWinner ? team2.getPartyLeader() : team1.getPartyLeader());
	...
	private void reward(Party party)
	{
		for(Player member : party.getPartyMembers())
		{
			...
			ItemFunctions.addItem(member, 13067, 30); // Fantasy Isle Coin
		}
	}
	...
	public void resurrectAtBase(Player player)
	{
		...
		player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(5660, 2)); // Battlefield Death Syndrome

		/*TODO [VISTALL]  Location pos;
	...
	private void removePlayer(Player player, boolean legalQuit)
	{
		...
		//TODO [VISTALL] player.teleToLocation(Location.findPointToStay(GvG.RETURN_LOC, 0, 150, ReflectionManager.DEFAULT.getGeoIndex()), 0);
	}

	/**
	 * @param party Handles the team withdraw from the area of event. Can only be called when !isActive
	 */
