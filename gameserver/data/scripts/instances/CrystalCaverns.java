...
public class CrystalCaverns extends Reflection
{
	...
	public void notifyTearsDead(NpcInstance npc)
	{
		...
		for(Creature c : npc.getAroundCharacters(600, 300))
			if(c.isPlayer())
				ItemFunctions.addItem(c.getPlayer(), 9697, 1); //Clear Crystal
	}
	...
	private void notifySteamStarted()
	{
		...
		for(Player p : getPlayers())
			p.altOnMagicUseTimer(p, SkillTable.getInstance().getSkillEntry(5239, 1));
	...
	public void notifyNextLevel(NpcInstance npc)
	{
		...
		for(Player p : getPlayers())
		{
			p.altOnMagicUseTimer(p, SkillTable.getInstance().getSkillEntry(5239, level));
	...
	public void notifyKechiDead(NpcInstance npc)
	{
		...
		for(Creature c : npc.getAroundCharacters(600, 300))
			if(c.isPlayer())
				ItemFunctions.addItem(c.getPlayer(), 9696, 1); //Red Crystal
		if(failureTimer != null)
		{
			failureTimer.cancel(false);
			failureTimer = null;
	...
	public void notifyDarnelDead(NpcInstance npc)
	{
		...
		for(Creature c : npc.getAroundCharacters(600, 300))
			if(c.isPlayer())
				ItemFunctions.addItem(c.getPlayer(), 9695, 1); // Blue Crystal
	}
		...
		public void onDeath(Creature self, Creature killer)
		{
			...
			else if(self.getNpcId() == gatekeeper_provo)
			{
				if(getAllByNpcId(gatekeeper_lohan, true).size() > 0)
					((NpcInstance) self).dropItem(killer.getPlayer(), 9699, 1, false); //Red Coral Key
			}
			else if(self.getNpcId() == gatekeeper_lohan)
			{
				if(getAllByNpcId(gatekeeper_provo, true).size() > 0)
					((NpcInstance) self).dropItem(killer.getPlayer(), 9698, 1, false); //Blue Coral Key
			}
