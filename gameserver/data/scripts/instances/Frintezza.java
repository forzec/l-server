...
public class Frintezza extends Reflection
{
	...
	private class SongEffectLaunched extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			frintezza.callSkill(SkillTable.getInstance().getSkillEntry(5008, _song), _targets, false);
		}
	}
	...
	private void block(NpcInstance npc, boolean flag)
	{
		...
		if(flag)
		{
			...
			npc.unblock();
		npc.setInvul(flag ? SpecialEffectState.TRUE : SpecialEffectState.FALSE);
	}
			...
			try
			{
				switch(_taskId)
				{
					...
					case 3:
						...
						blockAll(false);
						SkillEntry skill = SkillTable.getInstance().getSkillEntry(5017, 1);
	...
	private void blockUnblockNpcs(boolean block, int[] npcArray)
	{
		for(NpcInstance n : getNpcs())
			if(ArrayUtils.contains(npcArray, n.getNpcId()))
			{
				...
				{
					n.block();
					n.setInvul(SpecialEffectState.TRUE);
				}
				else
				{
					n.unblock();
					n.setInvul(SpecialEffectState.FALSE);
				}
			}
	}
	...
	private class DeathListener implements OnDeathListener
	{
		...
		public void onDeath(Creature self, Creature killer)
		{
			if(self.isNpc())
			{
				...
				{
					if(Rnd.chance(10))
						((NpcInstance) self).dropItem(killer.getPlayer(), DewdropItem, 1, false);
					//ToCheck: find easier way
					for(NpcInstance n : getNpcs())
