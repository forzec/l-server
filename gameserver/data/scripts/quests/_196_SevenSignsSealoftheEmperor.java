...
public class _196_SevenSignsSealoftheEmperor extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("shunaiman_q196_2.htm"))
		{
			st.setCond(4);
			st.playSound(SOUND_MIDDLE);
			player.sendPacket(SystemMsg.BY_USING_THE_SKILL_OF_EINHASADS_HOLY_SWORD_DEFEAT_THE_EVIL_LILIMS);
		...
		else if(event.equalsIgnoreCase("courtmagician_q196_2.htm"))
		{
			...
			player.sendPacket(SystemMsg.BY_USING_THE_COURT_MAGICIANS_MAGIC_STAFF_OPEN_THE_DOOR_ON_WHICH_THE_MAGICIANS_BARRIER_IS_PLACED);
		}
		else if(event.equalsIgnoreCase("free_anakim"))
		{
			if (ref.getInstancedZoneId() == izId) // precaution
			{
				...
				player.sendPacket(SystemMsg.IN_ORDER_TO_HELP_ANAKIM_ACTIVATE_THE_SEALING_DEVICE_OF_THE_EMPEROR_WHO_IS_POSSESSED_BY_THE_EVIL_MAGICAL_CURSE_MAGICAL_CURSE_IS_VERY_POWERFUL_SO_WE_MUST_BE_CAREFUL);
				ref.openDoor(door11);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == IasonHeine)
		{
			QuestState qs = player.getQuestState(195);
			if(cond == 0)
			{
