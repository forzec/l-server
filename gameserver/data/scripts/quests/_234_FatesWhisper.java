...
public class _234_FatesWhisper extends Quest
{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		int npcId = npc.getNpcId();
		int cond = st.getCond();
		if(cond == COND8 && ArrayUtils.contains(MOBS, npcId))
		{
			long count = st.getQuestItemsCount(BloodStainedCloth);
			if(st.getQuestItemsCount(WhiteCloth) > 0 && count < 30)
			{
	...
	public String onAttack(NpcInstance npc, QuestState st)
	{
		...
		{
			if(Rnd.chance(50))
				ChatUtils.say(npc, NpcString.WHO_DARES_TO_TRY_AND_STEAL_MY_NOBLE_BLOOD);
