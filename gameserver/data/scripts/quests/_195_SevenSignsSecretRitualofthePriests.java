...
public class _195_SevenSignsSecretRitualofthePriests extends Quest
{
		...
		else if(event.equalsIgnoreCase("open_door"))
		{
			if(ref != null && player.getTransformation() == 113 && st.getQuestItemsCount(GuardsoftheDawnIdentityCard) >= 1)
			{
				if(npc.getLoc().equals(new Location(-75695, 213537, -7128, 0)))
				{
					ref.openDoor(door1);
					ref.openDoor(door2);
					player.sendPacket(SystemMsg.BY_USING_THE_INVISIBLE_SKILL_SNEAK_INTO_THE_DAWNS_DOCUMENT_STORAGE);
				...
				else
				{
					ref.openDoor(door3);
					ref.openDoor(door4);
					player.sendPacket(SystemMsg.THE_DOOR_IN_FRONT_OF_US_IS_THE_ENTRANCE_TO_THE_DAWNS_DOCUMENT_STORAGE_APPROACH_TO_THE_CODE_INPUT_DEVICE);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			QuestState qs = player.getQuestState(194);
			if(cond == 0)
