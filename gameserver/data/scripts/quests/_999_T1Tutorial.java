...
public class _999_T1Tutorial extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("isle"))
		{
			...
			String title = npc == null ? "" : npc.getTitle() + " " + HtmlUtils.htmlNpcName(npc.getNpcId());
				...
				else if(step == 1 && st.getQuestItemsCount(t.item) == 0 && Ex <= 2)
				{
					if(st.getQuestItemsCount(BLUE_GEM) > 0)
					{
						...
						if(isMage)
						{
							...
							if(htmltext.isEmpty())
								htmltext = "<html><body>" + (npc.getTitle().isEmpty() ? "" : npc.getTitle() + " ") + HtmlUtils.htmlNpcName(npc.getNpcId()) + "<br>I am sorry. I only help warriors. Please go to another Newbie Helper who may assist you.</body></html>";
						}
						else
						{
							st.playTutorialVoice("tutorial_voice_026");
							...
							if(htmltext.isEmpty())
								htmltext = "<html><body>" + (npc.getTitle().isEmpty() ? "" : npc.getTitle() + " ") + HtmlUtils.htmlNpcName(npc.getNpcId()) + "<br>I am sorry. I only help mystics. Please go to another Newbie Helper who may assist you.</body></html>";
						}
					}
					else
					{
	...
	public static class DropGem extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			{
				_npc.dropItem(_st.getPlayer(), BLUE_GEM, 1, false);
