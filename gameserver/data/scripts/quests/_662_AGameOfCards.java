	...
	private static class CardGame
	{
		...
		public CardGame(int _player_id)
		{
			...
			for(int i = 0; i < cards.length; i++)
				cards[i] = "<a action=\"bypass -h npc_%objectId%_QuestEvent _662_AGameOfCards card" + i + "\">?</a>";
		}
		...
		private String finish(QuestState st)
		{
			...
			if(st.getQuestItemsCount(RED_GEM) >= 50)
				result += "<br><br><a action=\"bypass -h npc_%objectId%_QuestEvent _662_AGameOfCards 30845_10.htm\">Play Again!</a>";
			result += html_footer;
			Games.remove(player_id);
			return result;
		}
