...
public class FourSepulchersManager implements OnInitScriptListener
{
	...
	public static synchronized void tryEntry(NpcInstance npc, Player player)
	{
		...
		for(Player mem : player.getParty().getPartyMembers())
		{
			...
			if(ItemFunctions.getItemCount(mem, ENTRANCE_PASS) == 0)
			{
	...
	private static void entry(int npcId, Player player)
	{
		...
		for(Player member : player.getParty().getPartyMembers())
		{
			...
			ItemFunctions.deleteItem(member, ENTRANCE_PASS, 1);
			if(ItemFunctions.getItemCount(member, ANTIQUE_BROOCH) == 0)
				ItemFunctions.addItem(member, USED_PASS, 1);
			ItemFunctions.deleteItem(member, CHAPEL_KEY, ItemFunctions.getItemCount(player, CHAPEL_KEY));
		}
