	...
	public boolean useItem(Playable playable, ItemInstance item, boolean ctrl)
	{
		...
		if(blessedsoulSpiritConsumption == 0)
		{
			...
			if(isAutoSoulShot)
			{
				...
				player.sendPacket(new ExAutoSoulShot(SoulshotId, false), new SystemMessage(SystemMsg.THE_AUTOMATIC_USE_OF_S1_HAS_BEEN_DEACTIVATED).addItemName(spiritshotId));
				return false;
			}
			player.sendPacket(SystemMsg.YOU_MAY_NOT_USE_SPIRITSHOTS);
			return false;
		...
		//{
			...
			//{
			//	player.removeAutoSoulShot(SoulshotId);
			//	player.sendPacket(new ExAutoSoulShot(SoulshotId, false), new SystemMessage(SystemMsg.DUE_TO_INSUFFICIENT_S1_THE_AUTOMATIC_USE_FUNCTION_HAS_BEEN_DEACTIVATED).addItemName(spiritshotId));
			//	return false;
			//}
