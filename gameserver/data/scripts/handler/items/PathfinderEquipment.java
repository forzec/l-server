...
public class PathfinderEquipment extends SimpleItemHandler
{
			...
			for(int[] reward : _rewards)
				if(reward.length == 2)
					if(reward[0] > 0 && reward[1] > 0)
						ItemFunctions.addItem(player, reward[0], reward[1]);
			return true;
		}
	...
	public static boolean canBeExtracted(int itemId, Player player)
	{
		...
		{
			player.sendPacket(SystemMsg.YOUR_INVENTORY_IS_FULL, new SystemMessage(SystemMsg.S1_CANNOT_BE_USED_DUE_TO_UNSUITABLE_TERMS).addItemName(itemId));
			return false;
		}
		return true;
	}
